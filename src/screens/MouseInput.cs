...
namespace NonsensicalVideoGenerator
{
    public static class MouseInput
    {
...
#if ANDROID
        private static TouchLocation lastTouchState;
        ...
        public static MouseState CompatMouseState()
        {
            ...
            if (isTouching && touchState.Count > 0)
            {
                lastTouchState = touchState[0];
                if(!isFirstTouch)
                {
                    lastMouseState = new MouseState(lastTouchState.Position.ToPoint().X + oX, lastTouchState.Position.ToPoint().Y + oY, _mouseState.ScrollWheelValue, ButtonState.Released, ButtonState.Released, ButtonState.Released, ButtonState.Released, ButtonState.Released);
                    overrideMouseState = new MouseState(lastTouchState.Position.ToPoint().X + oX, lastTouchState.Position.ToPoint().Y + oY, _mouseState.ScrollWheelValue, ButtonState.Pressed, ButtonState.Released, ButtonState.Released, ButtonState.Released, ButtonState.Released);
                }
                else
                {
                    overrideMouseState = new MouseState(lastTouchState.Position.ToPoint().X + oX, lastTouchState.Position.ToPoint().Y + oY, _mouseState.ScrollWheelValue, ButtonState.Released, ButtonState.Released, ButtonState.Released, ButtonState.Released, ButtonState.Released);
                }
            }
            else
            {
